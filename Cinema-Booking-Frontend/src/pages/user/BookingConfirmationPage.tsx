  ...
  return (
    <div className="min-h-screen bg-dark-950 text-white">
      ...
      <main className="mx-auto max-w-3xl px-4 py-12 text-center sm:px-6 lg:px-8">
        ...
        {booking && (
          <div className="overflow-hidden rounded-xl border border-white/10 bg-dark-900 text-left shadow-2xl shadow-black/30">
                  ...
                  <div className="rounded-lg bg-white p-3">
                    <div className="grid h-24 w-24 grid-cols-5 gap-1">
                      {Array.from({ length: 25 }).map((_, index) => (
                        <span key={index} className={`${index % 2 === 0 || index % 7 === 0 ? 'bg-dark-950' : 'bg-slate-200'} rounded-sm`} />
                      ))}
                    </div>
                  </div>
                </div>
