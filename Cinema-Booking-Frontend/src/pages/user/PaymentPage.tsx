                ...
                return (
                  <button
                    ...
                    className={`rounded-lg border p-4 text-left transition ${
                      active ? 'border-primary-500 bg-primary-500/15 text-white' : 'border-white/10 bg-dark-950 text-slate-300 hover:border-white/30'
                    }`}
                  >
                    ...
                    <>
                      <div className="mx-auto mb-4 grid h-40 w-40 grid-cols-5 gap-1 rounded-lg bg-white p-3">
                        {Array.from({ length: 25 }).map((_, index) => (
                          <span key={index} className={`${index % 3 === 0 || index % 7 === 0 ? 'bg-dark-950' : 'bg-slate-200'} rounded-sm`} />
                        ))}
                      </div>
