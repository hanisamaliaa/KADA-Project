      ...
      {steps.map((step, index) => (
        <div key={step.id} className="flex items-center gap-2">
          <div className={`flex items-center gap-2 rounded-full px-2 py-1 sm:px-3 ${
            index <= currentStepIndex ? 'bg-primary-600 text-white' : 'bg-dark-800 text-slate-500'
          }`}>
          ...
          {index < steps.length - 1 && (
            <span className={`hidden h-px w-8 sm:block ${index < currentStepIndex ? 'bg-primary-500' : 'bg-dark-700'}`} />
          )}
        </div>
      ))}
    </div>
